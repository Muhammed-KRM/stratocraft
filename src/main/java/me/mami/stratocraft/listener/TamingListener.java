...
public class TamingListener implements Listener {
    ...
    public void onTamingRitual(PlayerInteractEvent event) {
        ...
        // Boss mu?
        BossManager.BossData bossData = bossManager != null ? bossManager.getBossData(targetEntity.getUniqueId())
                : null;
    ...
    private void handleBossTamingRitual(Player player, Block centerBlock, LivingEntity entity,
        ...
        if (!tamingManager.checkBossRitualPattern(centerBlock, bossType)) {
            player.sendMessage("§cRitüel deseni yanlış! " + bossManager.getBossDisplayName(bossType)
                    + " için doğru deseni yapmalısın.");
            return;
        }
    ...
    public void onRideCreature(PlayerInteractEntityEvent event) {
        ...
        if (player.isSneaking()) {
            // Aynı klan kontrolü
            me.mami.stratocraft.manager.ClanManager clanManager = Main.getInstance().getClanManager();
            if (clanManager != null) {
        ...
        for (org.bukkit.entity.Entity nearby : victim.getWorld().getNearbyEntities(victim.getLocation(), 20, 20, 20)) {
            ...
            if (attacker instanceof Player) {
                ...
                if (clanManager != null) {
                    ...
                    me.mami.stratocraft.model.Clan attackerClan = clanManager
                            .getClanByPlayer(attackerPlayer.getUniqueId());

...
public class SupplyDropManager {
    ...
    public void spawnSupplyDropAtLocation(Location targetLocation, Location groundLocation) {
        ...
        Location dropLoc = new Location(world, targetLocation.getBlockX() + 0.5, dropY, targetLocation.getBlockZ() + 0.5);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // Duman efekti (düşerken)
                try {
                    world.spawnParticle(org.bukkit.Particle.SMOKE_LARGE, 
                        fallingChest.getLocation(), 15, 0.5, 0.5, 0.5, 0.1);
                } catch (Exception e) {
    ...
    /**
     * Hava drop yere düştüğünde sandığı oluştur (EntityChangeBlockEvent'te çağrılır)
     */
    public void onSupplyDropLand(org.bukkit.event.entity.EntityChangeBlockEvent event) {
        ...
        if (fallingBlock == null || !fallingBlock.hasMetadata("SupplyDrop")) return;
        
        // Metadata kontrolü
        if (fallingBlock.getMetadata("SupplyDrop").isEmpty()) return;
        ...
        if (finalLoc == null || finalLoc.getWorld() == null) return;
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (finalLoc == null || finalLoc.getWorld() == null) return;
                if (finalLoc.getBlock() == null) return;
